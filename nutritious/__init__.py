'''
A modified fork of archived https://github.com/michaeljones/packed
'''
...
class Text(object):
    ...
    def compose(self, parser, indent=0):
        indent_str = indent * "    "
        return f"{indent_str}'{self.whitespace or ''}{self.value}'"
...
class Attribute(object):
    ...
    def compose(self, parser, indent=0):
        indent_str = indent * "    "
        return f"{indent_str}'{self.name}': {self.value.compose(parser)},"
...
class SelfClosingTag(object):
    ...
    def compose(self, parser, indent=0, first=False):
        ...
        text.append(
            "{indent}Element({paren_sep}{indent_plus}{name}{contents_sep}".format(
                indent=indent_str,
...
def to_html(entity):
    """Converts entity to output html with the ability to handle Element instances & unicode and lists
    of either."""
...
class Element(object):
    """Represents an HTML element. Packed translates the <a></a> into Element('a') with an optional
...
def litchi(func):
    ...
    better just returning Element objects
    """
...
def translate_file(py_file, output_path):
    """Reads & translates the provided .py file and writes the result to the provided output path."""
    pkd_contents = open(py_file, 'r').read()
